...
impl Storage for BasedirsStorage {
    ...
    async fn get_with_raw(&self, key: &str) -> Result<(Cache, Option<Bytes>)> {
        self.inner.get_with_raw(key).await
    }
...
mod test {
    ...
    async fn request_view_preserves_storage_capabilities() -> Result<()> {
        ...
        let empty_view = with_basedirs(Arc::clone(&storage), vec![]);
        assert!(!Arc::ptr_eq(&view, &storage));
        assert_eq!(view.basedirs(), request_basedirs);
        assert!(empty_view.basedirs().is_empty());
        ...
        let (cache, combined_raw) = view.get_with_raw(key).await?;
        assert!(matches!(cache, Cache::Hit(_)));
        assert_eq!(combined_raw.as_deref(), Some(raw.as_ref()));
