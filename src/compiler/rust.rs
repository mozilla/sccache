    ...
    T: CommandCreatorSync,
{
    async fn generate_hash_key(
        ...
        let os_string_arguments: Vec<(OsString, Option<OsString>)> = self.parsed_args.arguments
            .iter()
            .map(|arg| {
                (
                    arg.to_os_string(),
        ...
        trace!("[{}]: hashing {} externs", self.parsed_args.crate_name,
               self.parsed_args.externs.len());
        let abs_externs = self.parsed_args.externs.iter().map(|e| cwd.join(e)).collect::<Vec<_>>();
        let extern_hashes = hash_all(&abs_externs, pool);
        // Hash the contents of the staticlibs listed on the commandline.
        trace!("[{}]: hashing {} staticlibs", self.parsed_args.crate_name,
               self.parsed_args.staticlibs.len());
        let abs_staticlibs = self.parsed_args.staticlibs.iter()
                             .map(|s| cwd.join(s)).collect::<Vec<_>>();
        ...
        let emit_generates_only_metadata =
            !self.parsed_args.emit.is_empty()
            && self.parsed_args.emit.iter().all(|e| e == "metadata" || e == "dep-info");
