...
// Two default implemtations intended to conflict with invalid implementations

// No distributed client support, not valid
#[cfg(not(feature = "dist-client"))]
impl<T> CompilerPackager for T {
    fn write_pkg(self: Box<Self>, _f: File) -> Result<()> {
        bail!("Distributed client support not enabled")
    }
}
// Distributed client support but an unsupported platform
...
pub struct CompilerPackageBuilder {
...
#[cfg(feature = "dist-client")]
#[cfg(all(target_os = "linux", target_arch = "x86_64"))]
impl CompilerPackageBuilder {
    pub fn new() -> Self {
        CompilerPackageBuilder { dir_set: BTreeMap::new(), file_set: BTreeMap::new() }
    }
    ...
    pub fn into_compressed_tar<W: Write>(self, writer: W) -> Result<()> {
        ...
        let CompilerPackageBuilder { dir_set, file_set } = self;
