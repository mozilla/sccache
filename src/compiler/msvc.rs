...
use compiler::{clang, gcc, write_temp_file, Cacheable, CompileCommand, CompilerArguments};
use dist;
...
pub fn parse_arguments(
    ...
    for arg in ArgsIter::new(xclang_it, (&gcc::ARGS[..], &clang::ARGS[..])) {
        ...
        let args = match arg.get_data() {
            ...
            None => match arg {
                Argument::Raw(_) | Argument::UnknownFlag(_) => Some(&mut common_args),
                _ => unreachable!(),
            },
            Some(PassThrough(_)) | Some(PassThroughPath(_)) => Some(&mut common_args),
