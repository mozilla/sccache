    ...
    I: CCompilerImpl,
{
    async fn generate_hash_key(
        ...
        let too_hard_for_preprocessor_cache_mode =
            self.parsed_args.too_hard_for_preprocessor_cache_mode.is_some();
        if let Some(arg) = &self.parsed_args.too_hard_for_preprocessor_cache_mode {
            debug!("parse_arguments: Cannot use preprocessor cache because of {:?}", arg);
        }
                    ...
                    if !update_failed {
                        if let Some(key) = hit {
                            ...
                            let weak_toolchain_key =
                                format!("{}-{}", self.executable.to_string_lossy(), self.executable_digest);
        ...
        // executable path to try and prevent this
        let weak_toolchain_key = format!("{}-{}", self.executable.to_string_lossy(),
                                         self.executable_digest);
        Ok(HashResult {
            key,
            compilation: Box::new(CCompilation {
