...
use futures_03::{channel::mpsc, compat::*, future, prelude::*, ready, stream};
    ...
    C: CommandCreatorSync + 'static,
{
    ...
    type Error = Error;
    type Future = SFutureStd<Self::Response>;
    ...
    C: CommandCreatorSync,
{
    ...
    async fn compiler_info(
        ...
        let opt: Option<(PathBuf, FileTime)> = {
        ...
        let opt = match opt {
            Some(x) => Some(x), // TODO resolve the path right away
            None => {
...
impl<I: AsyncRead + AsyncWrite + Unpin> Stream for SccacheTransport<I> {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        Poll::Ready(
            ready!(Pin::new(&mut self.inner).poll_next(cx)).map(|s| s.map(Message::WithoutBody)),
        )
    }
}
