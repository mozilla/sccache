...
## Getting Started
The easiest way to use `sccache` with GitHub Actions is to use the [GitHub Action](https://github.com/marketplace/actions/rust-cache-using-sccache).
...
GHA cache storage will create many small caches with the same cache key, e.g. `SCCACHE_GHA_CACHE_TO` and `SCCACHE_GHA_CACHE_FROM`. These GHA caches are differentiated by their [_version_](https://github.com/actions/cache#cache-version). The GHA cache implementation in `sccache` calculates the cache version from the [`sccache` entry key](Caching.md), e.g. the source file path.
