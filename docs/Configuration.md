...
* `SCCACHE_SERVER_STARTUP_TIMEOUT_MS` how long the client waits for the server to start up, in milliseconds (default `10000`). The server checks its cache storage before it reports ready, so a slow or busy remote backend can need more. Environment variable takes precedence over `server_startup_timeout_ms` in the configuration file.
