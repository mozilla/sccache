...
* `SCCACHE_DISABLE_CACHE_RETRY` set to `1` to disable the cache retry. The retry will be done when getting temporary errors returned (network error, timeout, etc). By default, it will retry 3 times. The first retry will at 1s, second at 2s and third at 4s
