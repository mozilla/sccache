...
impl DistSystem {
    ...
    pub fn add_scheduler(&mut self) {
        ...
        wait_for(
            || {
                ...
                if matches!(self.scheduler_status(), SchedulerStatusResult { num_servers: 0, num_cpus: _ })
                {
                    Ok(())
                } else {
                    Err(format!("{:?}", status))
    ...
    pub fn wait_server_ready(&mut self, handle: &ServerHandle) {
        ...
        wait_for(
            || {
                let status = self.scheduler_status();
                if matches!(self.scheduler_status(), SchedulerStatusResult { num_servers: 1, num_cpus: _ })
                {
                    Ok(())
                } else {
                    Err(format!("{:?}", status))
